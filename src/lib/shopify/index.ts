...
function reshapeImages(images: Connection<Image>, productTitle: string) {
  const flattaened = removeEdgesAndNodes(images);

  return flattaened.map((image) => {
...
function reshapeProduct(
  ...
  filterHiddenProduct: boolean = true
) {
  if (
    !product ||
    (filterHiddenProduct && product.tags.includes(HIDDEN_PRODUCT_TAG))
  ) {
    return undefined;
  }
